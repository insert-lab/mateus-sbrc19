...
class App():
    ...
    def _verifyContent(self,content_name, provider_acount):
        try:
            self.contract_instance.functions.verifyContent(content_name,provider_acount).transact({'from':self.this_account,'gas':410000})
            status = self.contract_instance.functions.verifyContent(content_name,provider_acount).call()
            if (bool(status)):
    ...
    def _registerProvider(self,content_name,provider_acount):
        try:
            self.contract_instance.functions.registerAllowedProviders(content_name,provider_acount).transact({'from':self.this_account, 'gas': 410000})
    ...
    def _registerContent(self,content_name,producer_account):
        ...
        try:
            self.contract_instance.functions.registerContent(content_name,producer_account).transact({'from':self.this_account, 'gas': 410000})
    ...
    #         if c not in self.used_accounts:
    #             self.used_accounts.append(c)
    #             return c
    #     return None

    def getStatistics(self):
        if self.successfully_registered_contents != 0:
            return("RC: {0}\nRP:{1}\n".format(self.successfully_registered_contents,self.registered_providers)) # registered contents
        else:
            return("VS: {0}\nVF: {1}\nBD: {2}\n".format(self.verification_sucess_counter,self.verification_fail_counter,self.backoff_delay))
