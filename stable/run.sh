...
blockchain(){
  ...
  /usr/local/bin/vcmd -c "$core_dir/blockchain" -- /home/mateus/.npm_global/bin/ganache-cli -h 10.0.1.20 -p 7545 -a $((n_nodes+1)) > /dev/null &
...
start_ap(){
  ...
  cp -a ndn-content-validation "$core_dir/AP.conf"
  /usr/local/bin/vcmd -c "$core_dir/AP" -- bash "$core_dir"/AP.conf/ndn-content-validation/src/python/ap.sh "$core_dir" "$contract_address" &
}
